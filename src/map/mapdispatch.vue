...
import ImageWMS from 'ol/source/ImageWMS'
  export default {
    ...
    mounted() {
      ...
      let map = new Map({
        ...
        overlays: [
          new Overlay({
            id: 'popupD',
            element:document.getElementById('popDispatch'),
            autoPan: true,
            autoPanAnimation: {
              duration: 250
            },
      ...
      //地图描点
      this.$eventBus.$on('passDislist',data=>{
        let DisList = data[0],type = data[1];
        //先清空图层
        let disStation = this.mapUtil.getLayer("调度水库站");
        map.removeLayer(disStation);
        map.addLayer(layerCreator.tracinpoint(DisList, '调度水库站', feature=>{
          let attribute = feature.getProperties().attribute;
          let imgName = type == 'S'? 'ssleg':'ybleg';
          return new Style({
            image: new Icon(({
              src: `${process.env.BASE_URL}img/${imgName}.png`,
              scale: 1
            })),
            text: new Text({
              text: '●',
      ...
      map.on("pointermove", e => {
        ...
        let overlay = map.getOverlayById('popupD')
        let pixel = map.getEventPixel(e.originalEvent)
        if (map.hasFeatureAtPixel(pixel)) {
          map.forEachFeatureAtPixel(pixel, (feature, layer) => {
            if (layer.get('title') !== '调度水库站') return
            let attribute = feature.getProperties().attribute;
            this.$emit('popDispatchData',attribute);
      ...
      map.on("click", e => {
        ...
        map.forEachFeatureAtPixel(pixel, (feature, layer) => {
          let attribute =  feature.getProperties().attribute;
          this.$emit('showDispatchDetails',attribute);
        });
      })
      //点击左侧table联动控制地图上点
      this.$eventBus.$on("dispatchLinkage",x=>{
    ...
    destroyed() {
      ...
      this.$eventBus.$off('passDislist')
      this.$eventBus.$off('dispatchLinkage')
    }
  };
